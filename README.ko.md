...
* **주요국 핀 고정 필터 (Pinned Country Pills) & 3가지 뷰:** 주요 국가(한국, 일본, 미국, 중국 등) 퀵 필터 캡슐과 전체 드롭다운 셀렉터 제공. 그리드(미니멀 카드), 인터랙티브 세계 지도, 시계열 연표의 3가지 뷰로 탐색 가능.
...
│       ├── locationCoords.ts     # 지도 시각화용 지리 좌표 데이터베이스
...
│       └── translations.ts       # 4개 국어 UI 다국어 사전
